...
#      —— softmax 对噪声等输入会"过度自信"（实测分布外样本的 softmax 中位数高达 0.93），
#         而 logit 分值区分度更好（分布内中位数 11.2 vs 分布外 4.6），
#         与 Hendrycks 等提出的 Energy/MaxLogit 开集识别基线一致。
# 联合标定结果：真实测试图约 2.4% 会被误判为「其他」（1000 张测试集实测：
# softmax 条件单独命中 1.2%、logit 条件单独命中 2.3%，并集 2.4%）；
# 噪声/涂鸦类图片约四成被拦截（具体数值随分布外样本的构造方式波动）。
