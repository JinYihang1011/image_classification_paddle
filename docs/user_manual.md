...
> ⚠️ 注意：本系统只认识这 10 类物体。上传人物、食物、风景等**不属于这 10 类**的图片时，
> 系统会自动判定为 **「其他」** 并弹出黄色警告，而不是硬塞给你一个类别（详见第 7 节 Q5-2）。
> 少数与 CIFAR-10 图像风格接近的图片仍可能被误判，这是这类方法的已知局限。
...
| 文件 | 内容 |
|---|---|
| `comparison_table.md` | 各类干扰条件下（噪声/旋转/裁剪平移/低分辨率/中心裁剪）的准确率对比表 |
| `confusion_matrix.png` | 混淆矩阵热力图（颜色越深表示该类识别越准） |
| `classification_report.txt` | 每一类的精确率/召回率/F1 报告 |
| `ood_calibration.json` | 「其他」判定的阈值标定结果（真实图片误判率、干扰图片拦截率）—— 标定产物，不随 `eval.py` 每次重跑刷新 |
...
4. 如果这张图片不像这 10 类中的任何一类，页面会显示黄色警告，判定结果为 **「其他（other）」**
   并给出最高置信度，提示你不要采信这次结果（原因见第 7 节 Q5-2）。
...
④ 如果图片本来就不属于这 10 类，界面显示「其他」是**正常的**，不是故障 —— 见下一条 Q5-2。
